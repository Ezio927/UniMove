...
const activity: Activity = {
  ...
  organizer: { _id: 'owner-id', username: '组织者', email: 'owner@example.com' },
...
describe('Profile favorites', () => {
  it('announces that favorites are loading', () => {
    renderFavorites({
      favorites: [], favoriteIds: new Set(), loading: true, ready: false, error: null, errorKind: null, mutatingId: null,
  ...
  it('shows an empty state when there are no favorites', () => {
    renderFavorites({
      favorites: [], favoriteIds: new Set(), loading: false, ready: true, error: null, errorKind: null, mutatingId: null,
      toggleFavorite: vi.fn(), reload: vi.fn()
    });
  ...
  it('renders favorite activities with removal enabled', () => {
    renderFavorites({
      favorites: [activity as never], favoriteIds: new Set([activity._id]), loading: false, ready: true, error: null,
      errorKind: null, mutatingId: null, toggleFavorite: vi.fn(), reload: vi.fn()
    });
  ...
  it('shows a retryable error when favorites fail to load', () => {
    const reload = vi.fn();
    renderFavorites({
      favorites: [], favoriteIds: new Set(), loading: false, ready: false, error: '收藏加载失败详情',
      errorKind: 'load', mutatingId: null, toggleFavorite: vi.fn(), reload
    });
  ...
  it('labels a failed favorite removal as an action error', () => {
    ...
    let favoritesState = {
      favorites: [activity as never], favoriteIds: new Set([activity._id]), loading: false, ready: true,
      error: null as string | null, errorKind: null as 'load' | 'mutation' | null,
      mutatingId: null as string | null, toggleFavorite, reload: vi.fn()
    };
    vi.mocked(useProfile).mockReturnValue(profileState as never);
    ...
    favoritesState = { ...favoritesState, error: '取消收藏失败', errorKind: 'mutation' };
  ...
  it('uses the hook load error kind after an earlier removal attempt', () => {
    let favoritesState = {
      favorites: [activity as never], favoriteIds: new Set([activity._id]), loading: false, ready: true,
      error: null as string | null, errorKind: null as 'load' | 'mutation' | null,
    ...
    expect(screen.queryByRole('button', { name: activity.title })).not.toBeInTheDocument();
  });
});
